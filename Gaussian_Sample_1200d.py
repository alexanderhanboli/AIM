...
# MEANS = []
# for x in X:
#     MEANS.append(np.array([x]*700))
# VARIANCES = [0.05 ** 2 * np.eye(len(mean)) for mean in MEANS]

MEANS = [np.array([i, j] + list(np.zeros(698))) for i, j in itertools.product(range(-4, 5, 2),
                                                           range(-4, -1, 2))]
VARIANCES = [0.05 ** 2 * np.eye(len(mean)) for mean in MEANS]
...
def main():
    ...
    train_data['features'] = np.array([list(x) + [0] * 500 for x in train_data['features']])
    #valid_data['features'] = np.array([list(x) + [0] * 500 for x in valid_data['features']])
