...
class LAI_mg_cl_fgan(object):
    def __init__(self, args):
        ...
        self.model_name = "MixedGaussianExamplefGAN"
        self.z_dim = 2
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (X, _) in enumerate(self.train_loader):
                ...
                D_loss = -torch.mean(D_real - torch.exp(D_fake - 1))
                ...
                D_fake = self.D(self.FC(X_hat))
                z_mu, z_sigma = self.E(self.FC(X_hat))
                E_loss = torch.mean(torch.sum(0.5 * (z - z_mu) ** 2 * torch.exp(-z_sigma) + 0.5 * z_sigma + 0.5 * np.log(2*np.pi), 1))
